...
export function doDelete_sup(sup_id){
    ...
    Request().post("/index.php?action=removeSup",Qs.stringify(data))
    .then(res => {
...
export function doDelete_good(good_id){
    let data = {
        "id": good_id
    };

    Request().post("/index.php?action=removeGood",Qs.stringify(data))
    .then(res => {
        let response = res['data'];
        document.getElementById("content").innerHTML = response['message'];
    })
