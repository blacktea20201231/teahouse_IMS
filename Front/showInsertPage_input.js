import supplier_good from "./supplier_good.js"
import getGoodsBySupplierId from "./getGoodsBySupplierId.js"
import getUnitbyGoodID from "./getUnitbyGoodID.js"
import { doInsert_input } from "./doInsert.js"
import Request from "./Request.js"
import getInvbyGoodID from "./getInvbyGoodID.js"
import showinventoryRecords from "./showinventoryRecords.js"
export function showInsertPage_input() {


  Request().get("/index.php?action=getSup")
    .then(res => {
      let response = res['data']
      const rows = response['result']
      // switch(response['status']){
      //   case 200:
      let str = '<div class="container w-50 mt-5">'
       //供應商
      str += '<div class="input-group mb-3 mt-3">'
      str += '<div class="input-group-prepend">'
      str += '<label class="input-group-text" for="inputGroupSelect01">供&nbsp;&nbsp;應&nbsp;&nbsp;商</label>'
      str += '</div>'
      str += '<select class="custom-select" id="supplier_id">'
      str += '<option selected value=0>Choose...</option>'
      rows.forEach(element => {
        str += '<option value=' + element['id'] + '>' + element['name'] + '</option>'
      });
      str+="</select>"
      str+="</div>";
      //
      str += "<div ></div>"
      //商品
      str+='<div class="input-group mb-3">'
      str+='<div class="input-group-prepend">'
      str+='<label class="input-group-text" for="inputGroupSelect01">品項名稱</label>'
      str+='</div>'
      str+='<select class="custom-select" id="good" name="good">'//id
      str+= '</select>'
      str+='</div>'
       //當下庫存
       str+='<div class="input-group mt-3" id=curinv>'
       str+='<span class="input-group-text">當下庫存</span>'
       str+='<input type="text" class="form-control" id=curinv value="" readonly>'
       str+='</div>'
       //數量
      str+='<div class="input-group mt-3">'
      str+='<span class="input-group-text">數&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;量</span>'
      str+='<input type="text" class="form-control" id=amount>'//id
      str+='</div>'
      //價格
      str+='<div class="input-group mt-3">'
      str+='<span class="input-group-text">單&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;價</span>'
      str+='<input type="number" class="form-control" id=price min=0>'//id
      str+='</div>'
      //單位
      str+='<div class="input-group mt-3" id=unit>'
      str+='<span class="input-group-text">單&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;位</span>'
      str+='<input type="text" class="form-control" value="包" readonly id=good_unit>'
      str+='</div>'
      //有效期限
      str+='<div class="input-group mt-3">'
      str+='<span class="input-group-text" >有效期限</span>'
      str+='<input type="date" class="form-control" id="due">'//id
      str+='</div>'
     //備註
     str+='<br/>'
      str+='<div class="input-group" mb-3 mt-3>'
      str+='<span class="input-group-text">備&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;註</span>'
      str+='<input type="text" class="form-control" id="note">'
      str+='</div>'
      str+='</div>'
      str+='<div class="two_buttons d-flex justify-content-center mt-5">'
      str+='<button class="btn-dark mx-3" id=input>入庫</button>'
      str+='<button class="btn-dark mx-3" id=back>返回</button>'
      str+='</div>'
      

      document.getElementById("content").innerHTML = str;
      document.getElementById("supplier_id").onchange = function () {
        console.log(document.getElementById("supplier_id").value)
        getGoodsBySupplierId();
        document.getElementById('curinv').value=""
        getInvbyGoodID();
      }
      document.getElementById("good").onchange=function () {
        console.log(document.getElementById("good").value)
        getUnitbyGoodID();
        getInvbyGoodID();
      
      }
      document.getElementById('back').onclick=function () {
        showinventoryRecords();
      }
      document.getElementById("input").onclick = function () {
        console.log("input click")
        if(!document.getElementById('good').value||
        !document.getElementById('supplier_id').value||
        document.getElementById('supplier_id').value==0||
        !document.getElementById('amount').value||
        !document.getElementById('price').value||
        !document.getElementById('due').value)
        alert("請輸入完整資料")
        else{
          let good_names = document.getElementById('good')
          let good_name = good_names.options[good_names.selectedIndex].innerText
          let unit_name = document.getElementById('good_unit').value
          let amount = document.getElementById("amount").value
          // console.log(unit_name.value)
          const comfirmBox = window.confirm("您欲入庫：\n" + good_name + "：" + amount + unit_name 
          +"\n當前總額:$"+parseInt(amount)*parseInt(document.getElementById("price").value)+ 
          "\n請確認。")
          if (comfirmBox == true) {
            doInsert_input()
            //showinventoryRecords()
          }
        }     
       
       
      }
    })
    .catch(err => {
      document.getElementById("content").innerHTML = err; 
    })
  
}